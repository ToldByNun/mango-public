...
New file: write_file once (COMPLETE: handlers + HTTP + send + entry). Extend existing file: insert_lines at line N with a fenced multi-line block (not ±3 edit_file). Broken syntax OR mashed duplicate imports: write_file COMPLETE rewrite only — never insert/edit onto a broken file. Discord bots: `async def on_message`, `await channel.send(...)`, and `intents.message_content = True`.
