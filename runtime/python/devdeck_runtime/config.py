...
from devdeck_runtime.types import RuntimeConfig
...
def resolve_config_path(explicit: str | Path | None = None) -> Path:
    ...
    env_path = os.environ.get("DEVDECK_RUNTIME_CONFIG")
    if env_path:
...
def load_config(path: str | Path | None = None) -> RuntimeConfig:
    ...
    env_model_path = os.environ.get("DEVDECK_GGUF_MODEL_PATH")
    if env_model_path:
    ...
    if not config.model.path:
        raise ValueError(
            "model.path is empty. Set it in config.yaml or DEVDECK_GGUF_MODEL_PATH."
        )
